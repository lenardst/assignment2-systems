...
\noindent Notation: $F = 6BDD_{\mathrm{FF}}$ (single-device forward MLP FLOPs); backward $= 2F$. Each weight matrix is $DD_{\mathrm{FF}}$ FP16 elements ($2DD_{\mathrm{FF}}$ bytes). Ring costs per $S$-byte tensor: all-reduce $= 2(N{-}1)S/(NW)$; all-gather and reduce-scatter $= (N{-}1)S/(NW)$.

\question{Problem: Alternate Ring All-Reduce}
$N{-}1$ rounds, each sending one full $S$-byte tensor per device:
\[ T = (N-1)S/W, \]
about $N/2$ times the cost of ring reduce-scatter then all-gather (which moves $S/N$-sized chunks).
...
Three weight-gradient all-reduces of $2DD_{\mathrm{FF}}$ bytes each:
...
Set $T_{\mathrm{comm}} = T_{\mathrm{comp}} = 12BDD_{\mathrm{FF}}/(N_{\mathrm{DP}} C)$ and cancel:
\[ \frac{N_{\mathrm{DP}}-1}{W} = \frac{B}{C} \implies N_{\mathrm{DP}} \le 1 + \frac{BW}{C}. \]
...
Forward: 3 all-gathers of $2DD_{\mathrm{FF}}$ bytes each.
...
Hidden width $D_{\mathrm{FF}}/N_{\mathrm{TP}}$ per device:
...
Forward: 2 activation all-gathers ($S = 2BD_{\mathrm{FF}}$) + 1 output all-reduce ($S = 2BD$):
\[ T_{\mathrm{comm}}^{\mathrm{fwd}} = \frac{4(N_{\mathrm{TP}}{-}1)B(D+D_{\mathrm{FF}})}{N_{\mathrm{TP}} W}. \]
Backward dominated by all-reduce on $dx$ ($S = 2BD$):
...
$T_{\mathrm{FSDP}}$: 3 weight all-gathers (each $D \cdot D_{\mathrm{FF}}/N_{\mathrm{TP}}$ FP16 elements). $T_{\mathrm{TP}}$: 1 activation all-reduce ($B/N_{\mathrm{FSDP}} \times D$). With disjoint links:
\[ T_{\mathrm{comm}}^{\mathrm{fwd}} = \max\{T_{\mathrm{FSDP}}, T_{\mathrm{TP}}\}. \]

\subquestion{(c) Optimal device count (overlapped)}
Balance $T_{\mathrm{FSDP}} = T_{\mathrm{TP}}$: ratio $N_{\mathrm{FSDP}}/N_{\mathrm{TP}} \approx 2B/(3D_{\mathrm{FF}})$. Pin to compute: $N_{\mathrm{FSDP}} \approx 1 + BW/C$, $N_{\mathrm{TP}} \approx 1 + 3D_{\mathrm{FF}}W/(2C)$:
\[ N = N_{\mathrm{FSDP}} N_{\mathrm{TP}} \approx \frac{3BD_{\mathrm{FF}}W^{2}}{2C^{2}}. \]

\subquestion{(d) Optimal device count (no overlap)}
$T_{\mathrm{comm}} = T_{\mathrm{FSDP}} + T_{\mathrm{TP}}$. Optimizing the split:
\[ N \lesssim \frac{3BD_{\mathrm{FF}}W^{2}}{8C^{2}}, \]
about $4\times$ smaller than overlapped.
